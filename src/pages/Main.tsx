...
const Main: Component = () => {
  ...
  const [barberName] = createResource(async () => {
    const barberRes = await getCurrentBarber();
      ...
      <div class="flex flex-row">
        ...
        <div class="grow flex flex-col h-screen overflow-y-hidden bg-gray-100">
          <div class="flex-none flex flex-row lg:justify-end justify-between bg-sky-400 items-center border-b-2 border-rose-600">
            <button
              class="lg:hidden ml-2 block hover:bg-sky-600 rounded"
            ...
            <div class="relative p-3">
              <button
                ref={btnLoginInfo!}
                class="flex flex-row items-center gap-1 truncate hover:bg-sky-600 p-1 rounded"
                classList={{ 'bg-sky-600': loginInfoVisible() }}
                onClick={() => {
              ...
              <Show when={loginInfoVisible()}>
                <div
                  use:clickOutside={[() => setLoginInfoVisible(false), btnLoginInfo!]}
                  class="absolute z-20 top-16 right-3 bg-white xs:w-40 w-32 flex flex-col border-1 shadow rounded-sm py-2"
                >
                  <span class="px-2 text-xs font-semibold">{barberName()}</span>
                  <hr class="w-full h-px bg-gray-200 border-0 my-2" />
                  <A
                    class="p-2 hover:bg-slate-100 text-sm"
                    href="/profile"
                    onClick={() => setLoginInfoVisible(false)}
                  >
                    账户信息
                  </A>
                  <hr class="my-2 w-full h-px bg-gray-200 border-0" />
                  <button class="p-2 hover:bg-slate-100 text-sm text-left" onClick={handleLogout}>
                    退出登陆
                  </button>
                </div>
              </Show>

...
const BarberInfo: Component = () => {
    ...
    const save=async()=>{
        ...
        if(searchParams.barberId){
          let res=await updateBarber(searchParams.barberId , barber);
          if (res.ok){
            toast.success("修改成功");
          ...
          let res=await addBarber(barber);
    ...
    const onDeleteConfirm=async ()=>{
        let res=await deleteBarber(searchParams.barberId);
        if(res.ok){
            navigate("/barbers");
...
<div class="flex flex-col max-h-screen bg-white m-2 p-2 pb-4">
  <div class="flex flex-col xs:flex-row justify-between p-4 xs:items-center border-b">
    ...
    <div class="flex flex-col xs:flex-row gap-1 xs:gap-2">
        <Show when={searchParams.barberId}>
                <button class="w-24 py-2 border border-red-200 bg-white hover:border-red-500 rounded text-red-500" onClick={()=>setShowAlert(true)}>移除理发师</button>
        </Show>
  ...
  <div class="grow flex flex-col overflow-y-auto gap-4">
    <div class="flex flex-col mx-auto my-4"><span class="text-2xl  font-semibold">{searchParams.barberId ? '编辑理发师':'新增理发师'}</span></div>
    <div class="flex flex-col gap-6 justify-center max-w-3xl mx-auto w-full px-2">
      ...
      <div class="flex flex-col sm:flex-row justify-between gap-6 sm:gap-2">
        <div class="flex flex-col w-full gap-2">
          ...
          <input type="text" value={realName()} onInput={e=>setRealName(e.currentTarget.value)} class="w-full rounded border" placeholder="" />
        </div>
        <div class="flex flex-col w-full gap-2">
          <span class="text-gray-700 ">手机号</span>
          <input type="text" value={cellphone()} onInput={e=>setCellphone(e.currentTarget.value)} class="w-full rounded border" placeholder="" />
        </div>
      </div>
        <div class="flex flex-col sm:flex-row  justify-between gap-2">
          <div class="flex flex-col w-full gap-2">
            <span class="text-gray-700">邮箱</span>
            <input type="text" value={email()} onInput={e=>setEmail(e.currentTarget.value)} class="w-full rounded border" placeholder="" />
          </div>
        </div>
...
<Show when={showAlert()}>
    <Alert setShowAlert={setShowAlert} message='移除理发师' confirm={onDeleteConfirm}></Alert>
</Show>
</>
    );
};

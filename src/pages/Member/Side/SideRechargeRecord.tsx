import { type Component, Suspense, createSignal, createResource, Show, For, type Accessor, createEffect,on, createMemo } from "solid-js";
...
const SideRechargeRecord: Component<{
  lastRechargeTime:Accessor<number|undefined>,
  memberId:string,
}> = (props) => {
    ...
    const [newBatch,{refetch}]=createResource(
        page,
        async ()=>{
          return (await getRechargeRecordsByMemberId(props.memberId,page(),pageSize)).data
        }
    );
    ...
    createEffect(on(props.lastRechargeTime,()=>{
      data=[];
            ...
            <For each={allData()}>{row =>
              <div class="flex flex-col gap-2 mx-2 sm:mx-8">
                <div class="rounded-md border bg-white flex flex-col p-4 gap-2">
                  ...
                  <div class="text-xs">
                    由 
                    <span class="text-slate-500 text-sm" style="margin-left: 1px; margin-right: 1px;">{row.barberName}</span>
                    充值</div>

...
const RechargeRecords: Component = () => {
    ...
    const [rechargeRecordsResult]=createResource(
        ...
        async ()=>{
            ...
            return await getRechargeRecords(searchParams.currentPage as unknown as number?? 0,searchParams.pageSize as unknown as number ?? 20,searchParams.search);
        }
    );
    ...
    <div class="px-2 flex flex-col bg-white">
        ...
        <div class="w-full overflow-x-auto ">
            <table class="w-full text-left min-w-max">
                ...
                <tbody>
                <For each={rechargeRecordsResult()?.data}>{row=>
        ...
        <div class="flex flex-row gap-2 justify-center py-2">
            ...
            <Show when={ (Number((searchParams.currentPage as unknown as number ?? 0).toString())+1) * Number((searchParams.pageSize as unknown as number ?? 20).toString()) < Number(rechargeRecordsResult()?.totalCount.toString())}>
            <button class="rounded cursor-pointer disabled:cursor-default border border-slate-300 stroke-slate-300 hover:stroke-slate-500 hover:border-slate-500"  onClick={()=>{
                        setSearchParams({currentPage:Number((searchParams.currentPage as unknown as number ?? 0).toString()) + 1})
                    }}>

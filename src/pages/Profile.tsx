import { type Component, Suspense, Show, onMount } from "solid-js";
...
const Profile: Component = () => {
    ...
    onMount(async ()=>{
        let res=await getCurrentBarber();
        if(!res.ok) return;
    ...
    const save=async ()=>{
      ...
      let res=await updateInfo(newInfo);
        if (res.ok){
            toast.success("保存成功");
        } else {
        ...
        <div class="flex flex-col px-4 pb-4 gap-6">
          <div class="flex flex-col sm:flex-row justify-between gap-2">
            <div class="flex flex-col w-full">
              ...
              <input type="text" value={profile.realName} onInput={e=>setProfile({"realName":e.currentTarget.value})} class="w-full rounded border" placeholder="" />
            </div>
            <div class="flex flex-col w-full">
              <span class="text-gray-700 ">手机号</span>
              <input type="text" value={profile.cellphone} onInput={e=>setProfile({"cellphone":e.currentTarget.value})} class="w-full rounded border" placeholder="" />
            </div>
          </div>
          <div class="flex flex-col sm:flex-row ">
            <div class="flex flex-col w-full">
              <span class="text-gray-700">邮箱</span>
              <input type="text" value={profile.email} onInput={e=>setProfile({"email":e.currentTarget.value})}  class="w-full border rounded" />
            </div>
          </div>

          <div class="flex flex-col">
            <label class="flex flex-row w-full items-center text-gray-600 gap-2 mb-2" >
                <input checked={profile.willChangePassword} onInput={ _e => {
                ...
                }}  type="checkbox" /><div class="select-none text-sm text-slate-400">修改密码</div> 
            </label>
            <Show when={profile.willChangePassword}>
            <div class="flex flex-col w-full">
                    <span class="text-gray-600 ">旧密码</span>
                    <input type="password" value={profile.oldPassword} onInput={e=>setProfile({"oldPassword":e.currentTarget.value})} class="w-full rounded border" placeholder="" />
                </div>
                <div class="flex flex-col w-full">
                    <span class="text-gray-700 ">重新输入新密码</span>
                    <div class="flex flex-row items-center gap-1 relative">
                        <input ref={inputPassword!} type="password" value={profile.newPassword} onInput={e=>setProfile({"newPassword":e.currentTarget.value})}  class="w-full rounded border" placeholder="" />
                    <Show when={profile.isNewPasswordVisible} fallback={
        ...
        <div class="flex flex-col px-4 pb-4 gap-4">
            ...
            <div class="flex flex-col sm:flex-row ">
                <div class="flex flex-col w-full">
                  ...
                  <input type="text" value={profile.merchantName}  onInput={e=>setProfile({"merchantName":e.currentTarget.value})}  class="w-full rounded border" placeholder="" />
                </div>
              </div>
              <div class="flex flex-col sm:flex-row ">
                <div class="flex flex-col w-full">
                  <span class="text-gray-700">商户地址</span>
                  <input type="text" value={profile.merchantAddress}  onInput={e=>setProfile({"merchantAddress":e.currentTarget.value})}   class="w-full rounded border" placeholder="" />
                </div>
              </div>
            <div class="flex flex-col sm:flex-row ">
              <div class="flex flex-col w-full">
                <span class="text-gray-700">备注</span>
                <textarea class="w-full rounded" value={profile.merchantRemark}  onInput={e=>setProfile({"merchantRemark":e.currentTarget.value})} ></textarea>
              </div>
            </div>
          </div>

...
import { type Component, Suspense, Show, createSignal, createResource, onMount, onCleanup } from "solid-js";
...
const MemberInfo: Component= () => {
    ...
    const save=async ()=>{
        ...
        if(searchParams.memberId){
          let res=await updateMember(searchParams.memberId , member);
          if (res.ok){
            toast.success("修改成功");
            navigate("/members?memberId="+searchParams.memberId);
          } else {
          ...
          let res=await addMember(member);
          if (res.ok){
            if(searchParams.fromCalendarAdd){
                navigate("/appointment-add"+location.search+`&memberId=${res.data?.memberId}`+`&memberName=${encodeURIComponent(res.data?.realName??"")}`);
            } else {
              toast.success("添加成功")
              navigate("/members");
            }
    ...
    const onDeleteConfirm=async ()=>{
        let res=await deleteMember(searchParams.memberId);
        if(res.ok){
            navigate("/members");
            //TODO currentPage
...
<div class="flex flex-col max-h-screen bg-white">
  <div class="flex flex-col xs:flex-row justify-between p-4 xs:items-center border-b">
    ...
    <div class="flex flex-col xs:flex-row gap-1 xs:gap-2">
        <Show when={searchParams.memberId}>
            <button class="w-24 py-2 border border-red-200 bg-white hover:border-red-500 rounded text-red-500" onClick={()=>setShowAlert(true)}>删除会员</button>
        </Show>
  ...
  <div class="grow overflow-y-auto flex flex-col">
    <div class="flex flex-col mx-auto my-4"><span class="text-2xl  font-semibold">{searchParams.memberId ? '编辑会员':'新增会员'}</span></div>
    <div class="flex flex-col gap-5 m-2">
        ...
        <div class="flex flex-col px-4 pb-4 gap-4">
          <div class="flex flex-col sm:flex-row justify-between gap-2">
            <div class="flex flex-col w-full">
              ...
              <input type="text" value={memberInfo.realName} onInput={(e)=>setMemberInfo({"realName":e.currentTarget.value})} class="w-full rounded border" placeholder="" />
            </div>
            <div class="flex flex-col w-full">
              <span class="text-gray-700 ">手机号</span>
              <input type="text" value={memberInfo.cellphone} onInput={(e)=>setMemberInfo({"cellphone":e.currentTarget.value})} class="w-full rounded border" placeholder="" />
            </div>
          </div>
          <div class="flex flex-col sm:flex-row  justify-between gap-2">
            <div class="flex flex-col w-full">
              <span class="text-gray-700">性别</span>
              <select value={memberInfo.gender} onChange={e=>setMemberInfo({"gender":e.currentTarget.value})} class="w-full border rounded ">
                <option value="">全部</option>
                <option value="男" selected={memberInfo.gender==="男"}>男</option>
                <option value="女" selected={memberInfo.gender==="女"}>女</option>
              </select>
            </div>
            <div class="flex flex-col w-full">
                <span class="text-gray-700">出生日期</span>
                <input type="date" value={memberInfo.birthDay} onInput={e=>setMemberInfo({"birthDay":e.currentTarget.value})}  class="w-full border rounded" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-6 justify-center max-w-3xl w-full mx-auto border rounded-md">
        ...
        <div class="px-4 pb-4">
          <textarea class="w-full rounded" value={memberInfo.remark} onInput={e=>setMemberInfo({"remark":e.currentTarget.value})}></textarea>
        </div>
      </div>
...
<Show when={showAlert()}>
    <Alert setShowAlert={setShowAlert} message='删除会员' confirm={onDeleteConfirm}></Alert>
</Show>
</>
    );
};

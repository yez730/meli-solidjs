...
const Orders: Component = () => {
    ...
    const [ordersResult]=createResource(
        ...
        async ()=>{
            ...
            return await getOrders(searchParams.currentPage as unknown as number?? 0,searchParams.pageSize as unknown as number ?? 20,searchParams.search);
        }
    );
    ...
    <div class="px-2 flex flex-col bg-white">
        ...
        <div class="flex flex-col xs:flex-row xs:items-center xs:justify-between  gap-2 px-4 bg-slate-100 py-3 rounded">
            <div class="flex flex-col xs:flex-row gap-1 xs:gap-4">
                <div class="relative max-w-sm">
                    <input ref={inputSearch!} type="text" class="px-10 border border-slate-300 w-full rounded-3xl focus:ring-0 pr-8 focus:outline-none focus:border-slate-500 shadow-sm placeholder-slate-400" placeholder="客户姓名/手机号" value={searchParams.search ?? ""} onInput={e=>setSearchParams({currentPage:0,search:e.currentTarget.value})}  />
        ...
        <div class="w-full overflow-x-auto">
            <table class="w-full text-left min-w-max">
                ...
                <tbody>
                    <For each={ordersResult()?.data}>{row=>
                    <tr class="border-b hover:bg-gray-100">
                        ...
                        <td class="py-3 px-1">{Math.trunc(row.totalMinutes / 60) >0 ?`${ Math.trunc(row.totalMinutes / 60)} 小时${row.totalMinutes % 60===0?"": " "+row.totalMinutes % 60+ "分钟"  }`:`${row.totalMinutes % 60} 分钟`}</td>
        ...
        <Show when={ ordersResult()?.totalCount ?? 0>0}>
        <div class="flex flex-row gap-2 justify-center py-2">
            ...
            <Show when={ (Number((searchParams.currentPage as unknown as number ?? 0).toString())+1) * Number((searchParams.pageSize as unknown as number ?? 20).toString()) < Number(ordersResult()?.totalCount.toString())}>
            <button class="rounded cursor-pointer disabled:cursor-default border border-slate-300 stroke-slate-300 hover:stroke-slate-500 hover:border-slate-500"  onClick={()=>{
                        setSearchParams({currentPage:Number((searchParams.currentPage as unknown as number ?? 0).toString()) + 1})
                    }}>

...
const ServiceTypeInfo: Component = () => {
  ...
  const save = async () => {
    ...
    const serviceType: NewServiceType = {
      name: name(),
    ...
    if (searchParams.serviceTypeId) {
      const res = await updateServiceType(searchParams.serviceTypeId, serviceType);
      if (res.ok) {
        toast.success('修改成功');
  ...
  const onDeleteConfirm = async () => {
    const res = await deleteServiceType(searchParams.serviceTypeId);
    if (res.ok) {
  ...
  return (
    <>
        ...
        <div class="grow flex flex-col overflow-y-auto gap-4">
          <div class="flex flex-col mx-auto my-4">
            <span class="text-2xl  font-semibold">
              {searchParams.serviceTypeId ? '编辑服务' : '新增服务'}
            </span>
          </div>
          <div class="flex flex-col gap-6 justify-center max-w-3xl mx-auto w-full px-2">
            <div class="flex flex-col sm:flex-row justify-between gap-6 sm:gap-2">
              ...
              <div class="flex flex-col w-full gap-2">
                ...
                <select
                  onChange={(e) => setEstimatedDuration(e.currentTarget.value as unknown as number)}
                  class="w-full appearance-none blockbg-clip-padding bg-no-repeat border border-solid border-gray-500 rounded transition ease-in-out"
                >
                  <option value="" />
                  <For each={durationList}>
                    {(duration) => (
                      <option
                        ...
                        selected={estimatedDuration() === duration.value}
                      >
                        {duration.text}
                      </option>
                    )}
            ...
            <div class="flex flex-col w-full gap-2">
              ...
              <input
                type="number"
                value={normalPrize()}
                onInput={(e) => setNormalPrize(e.currentTarget.value as unknown as number)}
                class="w-full rounded border"
                placeholder=""
              />
            </div>
            <div class="flex flex-col w-full gap-2">
              ...
              <input
                type="number"
                value={memberPrize()}
                onInput={(e) => setMemberPrize(e.currentTarget.value as unknown as number)}
                class="w-full rounded border"
                placeholder=""
              />
            </div>
      ...
      <Show when={showAlert()}>
        <Alert setShowAlert={setShowAlert} message="删除服务" confirm={onDeleteConfirm} />
      </Show>
    </>
  );
};
